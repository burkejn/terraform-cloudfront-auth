###############################################################################
# Gating: build steps should not run in Atlantis by default
###############################################################################
variable "enable_local_build" {
  description = "When true, run local build steps (nvm/npm/zip) via null_resources. Keep false in Atlantis."
  type        = bool
  default     = false
}

###############################################################################
# Local nodejs dependency install (BUILD-ONLY)
###############################################################################
resource "null_resource" "provision_nodejs" {
  count = var.enable_local_build ? 1 : 0
  ...
  provisioner "local-exec" {
    command = <<-EOF
      ...
      # Option C: no 'curl | bash' (Atlantis runs /bin/sh and may not have bash)
...
resource "null_resource" "copy_source" {
  ...
  triggers = {
    # Must reference indexed resource when count is used
    build_resource = null_resource.provision_nodejs[0].id
  ...
  provisioner "local-exec" {
    command = <<-EOF
      ...
      if [ ! -d "build" ] && [ ! -L "build" ]; then
        ...
        cp ${data.local_file.build-js[0].filename} build/cloudfront-auth-${var.cloudfront_auth_branch}/build/build.js
...
###############################################################################
# Builds the Lambda zip artifact (BUILD-ONLY)
###############################################################################
resource "null_resource" "build_lambda" {
  ...
  triggers = {
    copy_source             = null_resource.copy_source[0].id
...
###############################################################################
# Copies the artifact to the root directory (BUILD-ONLY)
###############################################################################
resource "null_resource" "copy_lambda_artifact" {
  ...
  triggers = {
    build_resource = null_resource.build_lambda[0].id
  }
...
###############################################################################
# Workaround to sync file creation (BUILD-ONLY)
###############################################################################
data "null_data_source" "lambda_artifact_sync" {
  ...
  inputs = {
    ...
    trigger = null_resource.copy_lambda_artifact[0].id
  }
}
...
resource "aws_lambda_function" "default" {
  # In build mode we depend on the artifact copy. In deploy-only mode we don't.
  depends_on = var.enable_local_build ? [null_resource.copy_lambda_artifact] : []
  ...
  # Lazy conditional:
  # - build mode: use the synced path
  # - deploy-only mode (Atlantis): hash the prebuilt file at local.lambda_filename
  source_code_hash = var.enable_local_build
    ? filebase64sha256(data.null_data_source.lambda_artifact_sync[0].outputs["file"])
    : filebase64sha256(local.lambda_filename)
